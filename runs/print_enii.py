...
if __name__ == '__main__':
    ...
    main_config.add_argument('--serial', type=str, dest='serial', default='4,5')
    ...
    serial_str = 'e' + '%03d' % int(config.esb_serial) if is_ensemble else '%03d' % int(config.serial)
    ...
    for patient_idx in dfs_id:
        ...
        # Default setting for ensemble
        img_3d = np.zeros([num_slices, img_size, img_size, img_c])
        mask_3d = np.zeros([num_slices, img_size, img_size, img_c])

        cls_labels = np.zeros([num_slices, ])
        det_3d = np.zeros([num_slices, config.det_size, config.det_size, img_c])
        # img_names = np.zeros([num_slices, ], dtype=object)

        pmask_3d = np.zeros([num_ckpt, num_slices, img_size, img_size, img_c])
        pred_cls_probs = np.zeros([num_ckpt, num_slices])
        pdet_3d = np.zeros([num_ckpt, num_slices, config.det_size, config.det_size, img_c])
        pscore_3d = np.zeros([num_ckpt, num_slices, config.det_size, config.det_size, img_c])
        ...
        for ckpt_idx, ckpt_path in enumerate(all_ckpt_paths):
            ...
            for step, (img, mask, ste, det, name) in enumerate(val_db):
                ...
                if config.seq_len != 1:
                    pmask_3d[ckpt_idx, step * seq_len:(step + 1) * seq_len, :, :, :] = seg_prob.numpy()[0, :, :, :, :]
                    pdet_3d[ckpt_idx, step * seq_len:(step + 1) * seq_len, :, :, :] = det_prob.numpy()[0, :, :, :, :]
                    pscore_3d[ckpt_idx, step * seq_len:(step + 1) * seq_len, :, :, :] = det_score.numpy()[0, :, :, :, :]
            ...
            pred_mask_name = '_'.join(['pmask', show_name[config.model_name], patient_idx])
            pred_det_name = '_'.join(['pdet', show_name[config.model_name], patient_idx])

        pred_mask_3d, pred_det_3d = np.mean(pmask_3d, axis=0), np.mean(pdet_3d, axis=0)
        pred_score_3d = np.mean(pscore_3d, axis=0)
        # import pdb; pdb.set_trace()
        # val_ens1, val_ess1, val_rts1, val_scs1, val_wts1, val_ens2, val_ess2, val_rts2, val_scs2, val_wts2 = \
        #     metric.set_jafroc_seq_np(det_3d, pred_det_3d, pred_score_3d, img_names)
        ...
        pred_det_3d = np.transpose(np.squeeze(pred_det_3d), (1, 2, 0))

        print(patient_idx, np.max(pred_det_3d), np.min(pred_det_3d), np.median(pred_det_3d))
        ...
        pred_det_nii = nib.Nifti1Image(pred_det_3d, None, nib.Nifti1Header())
        ...
        nib.save(pred_mask_nii, os.path.join(case_patient_path, pred_mask_name))
        nib.save(pred_det_nii, os.path.join(case_patient_path, pred_det_name))
        ...
        print(os.path.join(case_patient_path, pred_mask_name))
        print(os.path.join(case_patient_path, pred_det_name))
