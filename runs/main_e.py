...
from data.setup_e import DataSettingV1, INFO_PATH
...
main_config.add_argument('--f_num', type=str, dest='f_num', default='8,16,32,64')
...
main_config.add_argument('--seq_len', type=int, dest='seq_len', default=64)
main_config.add_argument('--seq_interval', type=int, dest='seq_interval', default=32)
...
def training():
    ...
    try:
        for epoch in range(1, config.epoch + 1):
            ...
            for train_step, (img, mask, ste, name) in enumerate(train_db):
                img_patch = tf.split(tf.reshape(img, img.shape[1:6]), 7)
                ste_patch = tf.split(tf.squeeze(ste), 7)

                for img_idx, ste_idx in zip(img_patch, ste_patch):
                    with tf.GradientTape() as tape:
                        train_prob = model(img_idx)
                        train_loss_batch = cls_loss_fn(ste_idx, train_prob, cls_alpha, cls_gamma)

                        grads = tape.gradient(train_loss_batch, model.trainable_variables)
                        optimizer.apply_gradients(zip(grads, model.trainable_variables))

                    train_loss.append(train_loss_batch)
                    train_cx.extend(np.squeeze(train_prob.numpy()).tolist())
                    train_cy.extend(ste_idx)
            ...
            for val_step, (img, mask, ste, name) in enumerate(val_db):
                img_patch = tf.split(tf.reshape(img, img.shape[1:6]), 7)
                ste_patch = tf.split(tf.squeeze(ste), 7)

                for img_idx, ste_idx in zip(img_patch, ste_patch):
                    val_prob = model(img_idx)
                    val_loss_batch = cls_loss_fn(ste_idx, val_prob, cls_alpha, cls_gamma)

                    val_loss.append(val_loss_batch)
                    val_cx.extend(np.squeeze(val_prob.numpy()).tolist())
                    val_cy.extend(ste_idx)
            ...
            print('Epoch:%s '
                  'Train-Loss:%.4f AUC:%.3f Val-Loss:%.4f AUC:%.3f' %
...
def validation():
    ...
    for ckpt in all_ckpt_paths:
        ...
        for step, (img, mask, ste, name) in enumerate(val_db):
            img_patch = tf.split(tf.reshape(img, img.shape[1:6]), 7)
            ste_patch = tf.split(tf.squeeze(ste), 7)
            name_patch = tf.split(tf.squeeze(name), 7)

            for img_idx, ste_idx, name_idx in zip(img_patch, ste_patch, name_patch):
                with tf.GradientTape() as tape:
                    val_prob = model(img_idx)
                    val_loss_batch = cls_loss_fn(ste_idx, val_prob, cls_alpha, cls_gamma)
