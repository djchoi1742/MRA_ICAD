...
def validation():
    ...
    for ckpt in all_ckpt_paths:
        ...
        for step, (img, mask, ste, det, name) in enumerate(val_db):
            ...
            # det_loss_batch = metric.object_loss(det, det_prob)
            # det_loss_batch = metric.weighted_object_loss(det, det_prob)  # serial 4
            loss_vars = [seg_loss_batch, cls_loss_batch, det_loss_batch]
...
def show_slice_cam_2d(images, masks, pred_masks, stes, pred_stes, pred_scores, cam_stes, ggc_stes,
    ...
    for i in range(batch_size):
        ...
        if len(name_split) > 3:
            each_name = '_'.join([name_split[0], name_split[1], name_split[-1]])
            case_patient_path = os.path.join(case_path, '_'.join([name_split[0], name_split[1]]))
        else:
            each_name = '_'.join([name_split[0], name_split[-1]])
            case_patient_path = os.path.join(case_path, name_split[0])
        # if name_split[0] == 's002':
