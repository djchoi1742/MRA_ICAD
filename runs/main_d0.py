...
from data.setup_d import DataSettingV1, INFO_PATH  # for image as input
# from data.setup_f import DataSettingV1, INFO_PATH  # for predicted mask as input
...
main_config.add_argument('--model_name', type=str, dest='model_name', default='Model232')
        ...
        for epoch in range(1, config.epoch + 1):
            ...
            for train_step, (img, mask, ste, det, name) in enumerate(train_db):
                with tf.GradientTape() as tape:
                    cls_prob, det_prob = model(img)  # for image as input
                    # cls_prob, det_prob = model(mask)  # for predicted mask as input (Comparison)
                    ...
                    if config.mtl_mode:
                        ...
                        # train_loss_batch = metric.naive_sum_loss(loss_vars, config.cls_lambda, config.det_lambda)
            ...
            for val_step, (img, mask, ste, det, name) in enumerate(val_db):
                val_cls_prob, val_det_prob = model(img)  # for image as input
                # val_cls_prob, val_det_prob = model(mask)  # for predicted mask as input (Comparison)
            ...
            print('Epoch:%s Train-Cls:%.4f Det:%.4f FOM1:%.3f FOM2:%.3f AUC:%.3f '
                  'Val-Cls:%.4f Det:%.4f FOM1:%.3f FOM2:%.3f AUC:%.3f' %
                  (epoch, train_cls_loss_mean, train_det_loss_mean, train_fom1, train_fom2, train_auc,
...
def validation():
    ...
    show_slice_cam = show_slice_cam_seq if config.seq_len != 1 else show_slice_cam_2d
    ...
    for ckpt in all_ckpt_paths:
        ...
        for step, (img, mask, ste, det, name) in enumerate(val_db):
            ...
            with tf.GradientTape() as tape:
                ...
                # cam_layers, cls_prob, det_prob, det_gap, det_score = cam_model(mask)  # Comparison

            # print(np.max(det_prob))
            ...
            ens1, ess1, rts1, scs1, wts1, ens2, ess2, rts2, scs2, wts2 = jafroc_fn(det, det_prob, det_score, name)
...
def show_slice_cam_2d(images, masks, stes, pred_stes, pred_scores, cam_stes, ggc_stes,
...
def show_slice_cam_seq(images, masks,  stes, pred_stes, cam_stes, ggc_stes,
    ...
    for i in range(batch_size):
        ...
        for j in range(config.seq_len):
            ...
            show_det = dets.numpy()[i, j, :, :, 0]  # dets.numpy()[i, j, :, :, 0]
            show_pred_det = pred_dets.numpy()[i, j, :, :, 0]  # pred_dets.numpy()[i, j, :, :, 0]
            show_pred_bin_det = np.where(show_pred_det < 0.5, 0.0, 1.0)  # threshold: 0.9
            ...
            if is_png:
                ...
                # ax[1, 1].set_title('Stenosis Prob.: %.3f' % show_pred_ste, fontsize=6, color='darkgreen')

                ax[1, 2].imshow(show_mask, cmap='bone')
                ...
                # ax[1, 2].set_title('Stenosis Prob.: %.3f' % show_pred_ste, fontsize=6, color='darkgreen')
                ...
                # ax[2, 1].set_title('Stenosis: %d Prob: %.3f' % (show_ste, show_pred_ste), fontsize=6, color='green')
...
def show_slice_cam_3d(images, masks, stes, pred_stes, cam_stes, ggc_stes,
                      dets, pred_dets, pred_scores, name, is_png=True, num_rows=2, num_cols=4,
                      fig_size=(4 * 1.3, 2 * 1.3), save_path=plot_val_path):
    if not os.path.exists(save_path): os.makedirs(save_path)
    batch_size = images.shape[0]
    names, lbls, probs = [], [], []
    ...
    for i in range(batch_size):
        ...
        show_det = dets.numpy()[i, :, :, 0]  # dets.numpy()[i, j, :, :, 0]
        show_pred_det = pred_dets.numpy()[i, :, :, 0]  # pred_dets.numpy()[i, j, :, :, 0]
        show_pred_bin_det = np.where(show_pred_det < 0.5, 0.0, 1.0)  # threshold: 0.9
        ...
        rsz_show_det = skimage.transform.resize(show_det, images.shape[2:4])
        rsz_show_pred_det = skimage.transform.resize(show_pred_det, images.shape[2:4])
        ...
        for j in range(seq_len):
            ...
            if is_png:
                ...
                ax[0, 3].imshow(show_ggc_ste, cmap='bone')
                ax[0, 3].set_title('Guided Grad-CAM', fontsize=6, color='darkgoldenrod')
                ...
                # ax[1, 2].imshow(show_image, cmap='bone')
                # ax[1, 2].imshow(show_pred_mask, cmap='bone')
                ax[1, 2].imshow(rsz_show_pred_det, cmap=plt.cm.seismic, alpha=0.5, interpolation='nearest')
                ax[1, 2].set_title('Stenosis Prob.: %.3f' % show_pred_ste, fontsize=6, color='darkgreen')

                ax[1, 3].imshow(show_image, cmap='bone')
                ax[1, 3].imshow(show_cam_ste, cmap=plt.cm.seismic, alpha=0.5, interpolation='nearest')
                ax[1, 3].set_title('Stenosis: %d Prob: %.3f' % (show_ste, show_pred_ste), fontsize=6, color='green')
...
def _show_slice_cam_2d(images, masks, stes, pred_stes, cam_stes, ggc_stes,
                      dets, pred_dets, pred_scores, name, is_png=True,
